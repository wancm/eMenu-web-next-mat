...
export class MongodbMasterDataRepository implements MasterDataRepository {
    ...
    async loadResourcesAsync(identifier: string): Promise<Resources | undefined> {
    ...
    async saveResourcesAsync(resources: Resources): Promise<ObjectId> {

        const options = {
            projection: { _id: 0 },
        }
        const query = { identifier: resources.identifier }

        const found = await this.masterDataCollection.findOne(query, options)

        if (found) throw new Error(`${resources.identifier} document existed`)

        const result = await this.masterDataCollection.insertOne(resources)
